...
namespace PTUtility{
	...
	public:
		...
		//–³Œø‚ÈAABB‚ğ‚Â‚­‚é
		AABBS() : m_MinPos(1000.001, 1000.001, 1000.001), m_MaxPos(-1000.001, -1000.001, -1000.001){}

		//Å‘å‚ÆÅ¬ˆÊ’u‚ğw’è
		AABBS(const Vec3& max, Vec3& min) : m_MinPos(min), m_MaxPos(max){}
		...
		void InTriangle(
			...
			for (int i = 0; i < 3; i++){
				m_MaxPos(i) = (max[i] + 0.001 > m_MaxPos[i] ? max[i] + 0.001 : m_MaxPos[i]);
				m_MinPos(i) = (min[i] - 0.001 < m_MinPos[i] ? min[i] - 0.001 : m_MinPos[i]);
			}
